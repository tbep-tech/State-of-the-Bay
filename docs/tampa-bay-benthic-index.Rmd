...
source(here::here('docs/setup.R'))

maxyr <- 2019
seg <- c('OTB', 'HB', 'MTB', 'LTB', 'All (wt)')

tbbiscr <- anlz_tbbiscr(benthicdata)
tbbimed <- anlz_tbbimed(tbbiscr, bay_segment = seg, yrrng = c(1993, maxyr))

tbbidsc <- tbbimed %>% 
  filter(yr == maxyr) %>% 
  select(bay_segment, TBBICat) %>% 
  tibble::deframe()
```
...
In `r maxyr`, Old Tampa Bay received an overall assessment of `r tbbidsc['OTB']` (based on the amount of site scores as degraded, intermediate, or healthy), Hillsborough Bay was `r tbbidsc['HB']`, Middle Tampa Bay was `r tbbidsc['MTB']`, and Lower Tampa Bay was `r tbbidsc['LTB']`.  The bay-wide assessment of benthic condition was `r tbbidsc['All (wt)']`.
