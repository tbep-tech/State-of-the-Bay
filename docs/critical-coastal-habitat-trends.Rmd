...
```{r}
alluvout2(subtchgdat, fluccs, family = fml, maxyr = submax, width = 800, height = 800, mrg = 0)
```
...
```{r}
alluvout2(chgdat, fluccs, family = fml, maxyr = sprmax, width = 1000, height = 800, mrg = 95)
```
...
Areal coverage of seagrasses are tracked semi-annually to determine if they are increasing or decreasing over time.  From `r formals(sgsum_fun)$refyr` to `r submax`, seagrasses `r sgsums$refdir` by `r sgsums$refcmp` acres.  For the most recent years of seagrass maps, seagrasses `r sgsums$recdir` by `r sgsums$reccmp` acres from `r sgsums$penult` to `r submax`. Most significant seagrass losses were in Old Tampa Bay, a segment that has not been meeting water quality targets in recent years.
...
* [Visualization tools for land use change](https://shiny.tbep.org/landuse-change/) 
* [Seagrass monitoring in Tampa Bay](https://tampabay.wateratlas.usf.edu/seagrass-monitoring/)
* [Data products for seagrass](https://tbep.org/seagrass-assessment/)
* [Visualization tool for seagrass change](https://shiny.tbep.org/seagrass-analysis/)
